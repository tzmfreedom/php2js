...
class Converter extends \PhpParser\PrettyPrinterAbstract
{
    // Special nodes
    ...
    protected function pStmt_DeclareDeclare(Stmt\DeclareDeclare $node) {
        return 'var ' . $node->key . '=' . $this->p($node->value);
    }
    ...
    protected function pStmt_Foreach(Stmt\Foreach_ $node) {
        return 'for (var _i = 0; _i < ' . $this->p($node->expr) . '.length; _i++) {' . PHP_EOL
            . 'var ' . $this->p($node->valueVar) . ' = ' . $this->p($node->expr) . '[_i];'
            . $this->pStmts($node->stmts) . $this->nl . '}';
